...
var getSegmentData = function(lt, index, delta, options){
  ...
  */
  ...
  if(delta.toLowerCase() == 'all'){
    ...
    if((indexFlipper(index) == -1) && (options.right === true)){
      startVal = startVal.multiply(-1);
      endVal = endVal.multiply(-1);
      mag = mag.multiply(-1);
      rate = rate.multiply(-1);
    }
    return ee.Image.cat([startYear.add(1), endYear, startVal, endVal, mag, dur, rate, dsnr]).toArray(0);
...
exports.indexSelectPanel = function(){
  ...
  var indexList = ['NBR','NDVI','NDSI','TCB','TCG','TCW','B1','B2','B3','B4','B5','B7','NBRz','Band5z','ENC'];
